...
### Added
- **Dashboard workspace home (2026-05-12)**: tela `/` totalmente reformulada conforme design handoff `design_handoff_workspace_home`. Layout em grid 3 colunas (1.4fr 1fr 1fr), 2 linhas: tile "Meu trabalho" (span 2 rows) + Activity, Ciclos, Notificações, Próximos marcos. Topbar com saudação contextual (bom dia/tarde/noite), título "Início" e botão "personalizar". Cada tile é independente: carregamento paralelo via `useQueries`, skeleton por tile, empty states descritivos. Novo `issueService.myWork(memberId)` e hook `useMyWork` para buscar issues workspace-wide por `assignee_id`. Arquivos: `WorkspacePage.tsx` (reescrita), `issue.service.ts` (+myWork), `useIssues.ts` (+useMyWork).

- **Navbar — pesquisa global de issues (2026-05-12)**: o ícone de lupa no cabeçalho agora abre um modal de busca global (`GlobalSearch.tsx`). Ao digitar 2+ caracteres, faz uma requisição `GET /issues/?search=<query>` com debounce de 300ms e exibe os resultados com título, sequence ID e cor do estado. Clicar em um resultado navega diretamente para o detalhe da issue. Atalho de teclado `⌘K` / `Ctrl+K` também abre a busca. Arquivos: `Header.tsx` (botão com `onClick` e registro do atalho), novo `GlobalSearch.tsx`.
