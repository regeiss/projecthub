...
def _get_issues_map(project_id: str) -> dict:
    ...
    qs = Issue.objects.filter(project_id=project_id).select_related("state", "assignee").values(
        "id", "title", "sequence_id", "type", "due_date", "start_date", "estimate_days",
        "state__category", "state__name", "state__color",
        "assignee__name", "assignee__avatar_url",
    )
