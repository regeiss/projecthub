...
class PortfolioDashboardProjectSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            "open_issue_count", "days_late",
        ]

    def _metrics(self, obj) -> dict:
        if not hasattr(obj, '_cached_metrics'):
            from .rag import _get_progress_metrics
            obj._cached_metrics = _get_progress_metrics(obj)
    ...
    def get_open_issue_count(self, obj) -> int:
        m = self._metrics(obj)
    ...
    def get_days_late(self, obj) -> int:
        ...
        if not obj.end_date:
            return 0
        today = date.today()
        return max(0, (today - obj.end_date).days)
