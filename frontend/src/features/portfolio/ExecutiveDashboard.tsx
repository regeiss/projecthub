  ...
  return (
    <div>
      ...
      <div className="mb-6 grid grid-cols-4 gap-3">
        <EvmCard label="PV (Planned Value)" value={formatCurrency(totals?.pv ?? 0)} />
        <EvmCard label="EV (Earned Value)" value={formatCurrency(totals?.ev ?? 0)} />
        <EvmCard
          label="CPI"
          value={(totals?.cpi ?? 1).toFixed(2)}
          sub={(totals?.cpi ?? 1) >= 1 ? '✓ Abaixo do orçamento' : '⚠ Acima do orçamento'}
        />
        <EvmCard
          label="SPI"
          value={(totals?.spi ?? 1).toFixed(2)}
          sub={(totals?.spi ?? 1) >= 1 ? '✓ Adiantado' : '⚠ Atrasado'}
        />
      </div>
