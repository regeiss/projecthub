...
export function AddMemberModal({ open, onClose, workspaceSlug }: Props) {
  ...
  const { data: users = [], isLoading, error: searchError } = useKeycloakUsers(workspaceSlug, debouncedSearch)
  const addMember = useAddWorkspaceMember()
  const searchDetail = (searchError as { response?: { data?: { detail?: string } } } | null)?.response?.data?.detail
      ...
      <div className="space-y-4">
        ...
        {(search.length >= 2 || users.length > 0) && (
          ...
          >
            ...
            {!isLoading && searchDetail === 'keycloak_unavailable' && (
              <li role="alert" className="px-4 py-3 text-sm text-red-500 dark:text-red-400">
                Nao foi possivel consultar os usuarios do LDAP.
              </li>
            )}
            {!isLoading && !searchDetail && users.length === 0 && (
