...
function InviteStep({
  ...
  function updateEntry(id: string, user: KeycloakUser | null, role: 'member' | 'admin') {
    setEntries((prev) =>
      prev.map((e) => (e.id === id ? { ...e, user, role } : e)),
    )
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="space-y-1.5">
        ...
        <div className="flex items-center gap-2 rounded-md border border-gray-200 bg-gray-50 px-3 py-2">
          <code className="flex-1 truncate font-mono text-xs text-gray-500">
            {settingsUrl}
          </code>
          <button
            type="button"
...
export function OnboardingPage() {
  ...
  function handleWorkspaceDone(ws: Workspace) {
    setWorkspace(ws)
    setStep(1)
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-indigo-50/30 flex flex-col items-center justify-center p-6">
      ...
      <div className="w-full max-w-md bg-white rounded-xl border border-gray-200 shadow-sm p-8">
        ...
        {step === 0 && (
          <WorkspaceCreateForm onSuccess={handleWorkspaceDone} />
        )}
